...
import ShiftSchedulingConfigView from "./ShiftSchedulingConfigView.tsx";
import ShiftJobsTabView from "./shift-jobs/ShiftJobsTabView.tsx";
import WeeklyShiftSchedulePage from "./WeeklyShiftSchedulePage.tsx";

const ShiftSchedulingPage: React.FC = () => {
  const [activeTab, setActiveTab] = useState<"board" | "config" | "jobs">("board");
    ...
    <div className="flex flex-col h-full overflow-hidden bg-slate-50/50">
      ...
      <div className="bg-white border-b border-slate-200 px-8 shrink-0 shadow-sm z-10">
        <div className="flex items-center h-16 gap-10">
          ...
          >
            <span className={`material-symbols-outlined text-[22px] transition-transform duration-300 ${activeTab === "board" ? "scale-110" : "group-hover:scale-110"}`}>
              calendar_view_week
            </span>
            Bảng xếp ca
            {activeTab === "board" && (
          ...
          >
            <span className={`material-symbols-outlined text-[22px] transition-transform duration-300 ${activeTab === "jobs" ? "scale-110" : "group-hover:scale-110"}`}>
              work_history
            </span>
            Công việc
            {activeTab === "jobs" && (
