...
import { employeeCategoryService, type ResignationReason } from "../../../services/employeeCategoryService";

interface ResignationFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  initialData: ResignationReason | null;
}

const ResignationFormModal: React.FC<ResignationFormModalProps> = ({
  isOpen,
  onClose,
  onSuccess,
  initialData
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name.trim()) {
      setError("Vui lòng nhập tên lý do nghỉ việc.");
      return;
    }
    ...
    try {
      if (initialData) {
        await employeeCategoryService.updateResignationReason(initialData.id, formData);
      } else {
        await employeeCategoryService.createResignationReason(formData);
      }
      onSuccess();
    } catch (e) {
      ...
      <div className="relative w-full max-w-lg bg-white rounded-[32px] shadow-[0_20px_50px_rgba(0,0,0,0.15)] overflow-hidden animate-in zoom-in-95 duration-300">
        <div className="px-8 py-6 border-b border-slate-50 flex items-center justify-between">
          <h3 className="text-xl font-bold text-slate-900">
            {initialData ? "Chỉnh sửa lý do nghỉ việc" : "Tạo mới lý do nghỉ việc"}
          </h3>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div>
            <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Tên lý do nghỉ việc *</label>
            <input 
              type="text" 
              value={formData.name}
              ...
              placeholder="Ví dụ: Hết hạn hợp đồng, Lý do cá nhân..."
              autoFocus
            />
          ...
          <div>
            ...
            <textarea 
              ...
              placeholder="Mô tả chi tiết về lý do nghỉ việc..."
            />
          </div>
