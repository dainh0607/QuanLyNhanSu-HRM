...
import { employeeCategoryService, type RewardType } from "../../../services/employeeCategoryService";

interface RewardFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  initialData: RewardType | null;
}
...
const RewardFormModal: React.FC<RewardFormModalProps> = ({
  ...
  // SỬA LỖI REGEX XỬ LÝ CHỮ Đ VÀ CHUẨN HÓA TIẾNG VIỆT
  const generateSlug = (str: string) => {
    return str
      .normalize("NFD")
      .replace(/[\u0300-\u036f]/g, "") // Loại bỏ dấu
      .replace(/đ/g, "d")
      .replace(/Đ/g, "D") // Fix lỗi chữ Đ
  ...
  const handleNameChange = (name: string) => {
    ...
    const suffix = generateSlug(name);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name.trim()) return setError("Vui lòng nhập tên khen thưởng.");
    ...
    try {
      ...
      if (initialData) {
        await employeeCategoryService.updateRewardType(initialData.id, payload);
      } else {
        await employeeCategoryService.createRewardType(payload);
      }
      onSuccess();
    } catch (e) {
    ...
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-[32px] shadow-[0_20px_50px_rgba(0,0,0,0.15)] overflow-hidden animate-in zoom-in-95 duration-300">
        <div className="px-8 py-6 border-b border-slate-50 flex items-center justify-between bg-emerald-50/30">
          <div>
            <h3 className="text-xl font-bold text-slate-900">
              {initialData ? "Cập nhật khen thưởng" : "Thêm hình thức khen thưởng"}
            </h3>
            <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mt-1">Chuẩn hóa danh mục phúc lợi & biến số lương</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div>
            <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Tên khen thưởng *</label>
            <input 
              type="text" 
              value={formData.name}
              ...
              placeholder="Ví dụ: Thưởng đạt KPI, Thưởng dự án..."
              autoFocus
            />
          </div>
          ...
          <div>
            <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2">
              Từ khóa định danh
              <span className="material-symbols-outlined text-[14px] text-slate-300 cursor-help" title="Dùng làm biến số trong công thức cộng tiền lương">help</span>
            </label>
            <div className="flex items-stretch overflow-hidden rounded-2xl border border-slate-100 bg-slate-50 group focus-within:ring-4 focus-within:ring-emerald-500/10 focus-within:bg-white focus-within:border-emerald-100 transition-all">
              ...
              <input 
                ...
                onChange={(e) => setFormData({ ...formData, suffix: generateSlug(e.target.value) })}
                className="flex-1 px-4 py-3 bg-transparent text-sm font-black text-emerald-600 focus:outline-none"
                placeholder="HAU_TO"
              />
            </div>
            <p className="mt-2 text-[10px] text-slate-400 font-medium italic">* Ghép hoàn chỉnh: <span className="font-black text-emerald-500">{PREFIX}{formData.suffix || "..." || ""}</span></p>
          </div>

          <div>
            <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Ghi chú</label>
            <textarea 
              ...
              placeholder="Quy định điều kiện nhận khen thưởng..."
            />
          </div>
