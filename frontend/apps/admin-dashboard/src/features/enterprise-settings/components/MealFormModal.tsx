...
import { employeeCategoryService, type MealType } from "../../../services/employeeCategoryService";
...
const MealFormModal: React.FC<MealFormModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name.trim()) return setError("Vui lòng nhập tên khẩu phần ăn.");
    ...
    try {
      ...
      if (initialData) {
        await employeeCategoryService.updateMealType(initialData.id, payload);
      } else {
        await employeeCategoryService.createMealType(payload);
      }
      onSuccess();
    } catch (e) {
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white rounded-[32px] shadow-[0_20px_50px_rgba(0,0,0,0.15)] overflow-hidden animate-in zoom-in-95 duration-300">
        <div className="px-8 py-6 border-b border-slate-50 flex items-center justify-between bg-blue-50/30">
          <div>
            <h3 className="text-xl font-bold text-slate-900">
              {initialData ? "Cấu hình suất ăn" : "Thêm khẩu phần ăn mới"}
            </h3>
            <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mt-1">Quản lý định mức suất ăn & phụ cấp ca</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          <div>
            <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Tên khẩu phần ăn *</label>
            <input 
              type="text" 
              value={formData.name}
              ...
              placeholder="Ví dụ: Ăn trưa, Ăn giữa ca..."
              autoFocus
            />
          </div>
          ...
          <div>
            <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-2">
              Từ khóa định danh
              <span className="material-symbols-outlined text-[14px] text-slate-300 cursor-help" title="Dùng để map vào cấu hình Ca làm việc và Bảng lương">help</span>
            </label>
            <div className="flex items-stretch overflow-hidden rounded-2xl border border-slate-100 bg-slate-50 group focus-within:ring-4 focus-within:ring-emerald-500/10 focus-within:bg-white focus-within:border-emerald-100 transition-all">
              ...
              <input 
                ...
                className="flex-1 px-4 py-3 bg-transparent text-sm font-black text-blue-600 focus:outline-none"
                placeholder="HAU_TO"
              />
            </div>
            <p className="mt-2 text-[10px] text-slate-400 font-medium italic">* Mã định danh: <span className="font-black text-blue-500">{PREFIX}{formData.suffix || "..." || ""}</span></p>
          </div>
          ...
          <div>
            ...
            <textarea 
              ...
              placeholder="Ghi chú về định mức tiền hoặc thời gian cấp phát..."
            />
          </div>
